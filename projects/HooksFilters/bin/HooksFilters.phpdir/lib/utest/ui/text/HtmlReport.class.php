...
class utest_ui_text_HtmlReport implements utest_ui_common_IReport{
	...
	public function jsScript() {
		...
		{
			$GLOBALS['%s']->pop();
			return "function utestTooltip(ref, text) {\x0D\x0A\x09var el = document.getElementById(\"utesttip\");\x0D\x0A\x09if(!el) {\x0D\x0A\x09\x09var el = document.createElement(\"div\")\x0D\x0A\x09\x09el.id = \"utesttip\";\x0D\x0A\x09\x09el.style.position = \"absolute\";\x0D\x0A\x09\x09document.body.appendChild(el)\x0D\x0A\x09}\x0D\x0A\x09var p = utestFindPos(ref);\x0D\x0A\x09el.style.left = p[0];\x0D\x0A\x09el.style.top = p[1];\x0D\x0A\x09el.innerHTML =  text;\x0D\x0A}\x0D\x0A\x0D\x0Afunction utestFindPos(el) {\x0D\x0A\x09var left = 0;\x0D\x0A\x09var top = 0;\x0D\x0A\x09do {\x0D\x0A\x09\x09left += el.offsetLeft;\x0D\x0A\x09\x09top += el.offsetTop;\x0D\x0A\x09} while(el = el.offsetParent)\x0D\x0A\x09return [left, top];\x0D\x0A}\x0D\x0A\x0D\x0Afunction utestRemoveTooltip() {\x0D\x0A\x09var el = document.getElementById(\"utesttip\")\x0D\x0A\x09if(el)\x0D\x0A\x09\x09document.body.removeChild(el)\x0D\x0A}";
		}
		$GLOBALS['%s']->pop();
	}
...
function utest_ui_text_HtmlReport_0(&$»this, &$infos, &$time, &$v) {
		$GLOBALS['%s']->push('utest.ui.text.HtmlReport:lambda_0');
		$»spos = $GLOBALS['%s']->length;
...
function utest_ui_text_HtmlReport_1(&$»this, &$addNL, &$parts, &$stack) {
		$GLOBALS['%s']->push('utest.ui.text.HtmlReport:lambda_1');
		$»spos = $GLOBALS['%s']->length;
	if($addNL) {
		return "\x0A";
	} else {
