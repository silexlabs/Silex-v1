...
package org.silex.wysiwyg.sequencer.state_classes 
{
	...
	/**
	 * a state designed to add a legacy component (component with editable properties without a descriptor,
	 * located in the media folder) to a layer using the library
	 */
	public class AddLegacyComponentLibraryState extends StateBaseAddComponent
	{

		/**
		 * opens the AddComponent library and set it's filters to "swf" as we want to add a legacy component which
		 * can for now only be a swf file
		 */
		public function AddLegacyComponentLibraryState(silexAdminApi:SilexAdminApi, toolCommunication:ToolCommunication, data:Object) 
		{
		...
		/**
		 * When the user selects a legacy component in the library, we add it using only it's url.
		 * The lack of className will make Silex fallback on the editableProperties array for this component
		 * 
