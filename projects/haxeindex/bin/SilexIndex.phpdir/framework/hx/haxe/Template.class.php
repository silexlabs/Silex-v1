...
class haxe_Template {
	...
	public function makeExpr2($l) {
		...
		case "(":{
			...
			return haxe_Template_1($this, $e1, $e2, $l, $p, $p1, $p2);
		}break;
		case "!":{
			$e = $this->makeExpr($l);
			return array(new _hx_lambda(array(&$e, &$l, &$p), "haxe_Template_2"), 'execute');
		}break;
		case "-":{
			$e = $this->makeExpr($l);
			return array(new _hx_lambda(array(&$e, &$l, &$p), "haxe_Template_3"), 'execute');
		}break;
		}
	...
	public function makePath($e, $l) {
		...
		return $this->makePath(array(new _hx_lambda(array(&$e, &$f, &$field, &$l, &$p), "haxe_Template_4"), 'execute'), $l);
	}
	public function makeConst($v) {
		...
		if(_hx_char_code_at($v, 0) === 34) {
			...
			return array(new _hx_lambda(array(&$str, &$v), "haxe_Template_5"), 'execute');
		}
		if(haxe_Template::$expr_int->match($v)) {
			$i = Std::parseInt($v);
			return array(new _hx_lambda(array(&$i, &$v), "haxe_Template_6"), 'execute');
		}
		if(haxe_Template::$expr_float->match($v)) {
			$f = Std::parseFloat($v);
			return array(new _hx_lambda(array(&$f, &$v), "haxe_Template_7"), 'execute');
		}
		$me = $this;
		return array(new _hx_lambda(array(&$me, &$v), "haxe_Template_8"), 'execute');
	}
	public function parseExpr($data) {
		...
		return array(new _hx_lambda(array(&$data, &$e, &$expr, &$l, &$s), "haxe_Template_9"), 'execute');
	}
