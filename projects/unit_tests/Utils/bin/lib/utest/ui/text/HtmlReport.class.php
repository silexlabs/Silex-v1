...
class utest_ui_text_HtmlReport implements utest_ui_common_IReport{
	...
	public function blockNumbers($buf, $stats) {
		...
		$»spos = $GLOBALS['%s']->length;
		{
			$x = "<div class=\"" . $this->cls($stats) . "bg statnumbers\">";
			if(is_null($x)) {
				$x = "null";
			} else {
				if(is_bool($x)) {
	...
	public function addFixture($buf, $result, $name, $isOk) {
		...
		{
			$x = "<li class=\"fixture\"><div class=\"li\">";
			if(is_null($x)) {
				$x = "null";
			} else {
				if(is_bool($x)) {
		...
		{
			$x = "<span class=\"" . $this->cls($result->stats) . "bg fixtureresult\">";
			if(is_null($x)) {
				$x = "null";
			} else {
				if(is_bool($x)) {
	...
	public function addPackages($buf, $result, $isOk) {
		...
		{
			$x = "<ul id=\"utest-results-packages\">\x0A";
			if(is_null($x)) {
				$x = "null";
			} else {
				if(is_bool($x)) {
	...
	public function getHeader() {
		...
		{
			$x = "<h1 class=\"" . $this->cls($this->result->stats) . "bg header\">" . $msg . "</h1>\x0A";
			if(is_null($x)) {
				$x = "null";
			} else {
				if(is_bool($x)) {
		...
		{
			$x = " performed on <strong>" . utest_ui_text_HtmlReport::$platform . "</strong>, executed in <strong> " . $time . " sec. </strong></div >\x0A ";
			if(is_null($x)) {
				$x = "null";
			} else {
				if(is_bool($x)) {
	...
	public function getTrace() {
		...
		{
			$x = "<div class=\"trace\"><h2>traces</h2><ol>";
			if(is_null($x)) {
				$x = "null";
			} else {
				if(is_bool($x)) {
		...
		}
		{
			...
			while($_g < $_g1->length) {
				...
				{
					$x = "<span class=\"tracepos\" onmouseover=\"utestTooltip(this.parentNode, '" . $stack . "')\" onmouseout=\"utestRemoveTooltip()\">";
					if(is_null($x)) {
						$x = "null";
					} else {
						if(is_bool($x)) {
				...
				{
					$x = str_replace("\x0A", "<br/>\x0A", trim($t->msg));
					if(is_null($x)) {
						$x = "null";
					} else {
						if(is_bool($x)) {
				...
				{
					$x = "</span><div class=\"clr\"></div></div></li>";
					if(is_null($x)) {
						$x = "null";
					} else {
						if(is_bool($x)) {
