		...
		while($»it->hasNext()) {
			...
			default:{
				...
				if($nodeValue !== "") {
					$duplicatedXml = null;
					$duplicatedXml = null;
					if($child1->nodeType == Xml::$PCData) {
						$duplicatedXml = Xml::parse("<a>" . $child1->getNodeValue() . "</a>")->firstElement()->firstChild();
					}
					if($child1->nodeType == Xml::$CData) {
						$duplicatedXml = Xml::parse("<a><![CDATA[" . $child1->getNodeValue() . "]]></a>")->firstElement()->firstChild();
					}
				...
				while($»it->hasNext()) {
					...
					try {
						...
						$e = $_ex_;
						{
							...
							haxe_Log::trace("FC:" . $xml->firstChild(), _hx_anonymous(array("fileName" => "XmlUtils.hx", "lineNumber" => 253, "className" => "org.silex.core.XmlUtils", "methodName" => "xml2DynamicRecursive")));
							haxe_Log::trace($node->toString(), _hx_anonymous(array("fileName" => "XmlUtils.hx", "lineNumber" => 254, "className" => "org.silex.core.XmlUtils", "methodName" => "xml2DynamicRecursive")));
						}
					}
					unset($e);
				}
