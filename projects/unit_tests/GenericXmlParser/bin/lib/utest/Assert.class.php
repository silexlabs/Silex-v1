...
class utest_Assert {
	...
	static function sameAs($expected, $value, $status) {
		...
		$c = $裨->params[0];
		{
			...
			if(Std::is($expected, _hx_qtype("Array"))) {
				if($status->recursive || $status->path === "") {
					if(!_hx_equal(_hx_len($expected), _hx_len($value))) {
						$status->error = "expected " . _hx_len($expected) . " elements but they were " . _hx_len($value) . (utest_Assert_5($c, $cexpected, $cvalue, $expected, $status, $texpected, $tvalue, $value));
						{
							$GLOBALS['%s']->pop();
							return false;
						}
					...
					}
					{
						...
						while($_g1 < $_g) {
							...
							if(ord($ebytes->b[$i]) !== ord($vbytes->b[$i])) {
								$status->error = "expected byte " . ord($ebytes->b[$i]) . " but wss " . ord($ebytes->b[$i]) . (utest_Assert_9($_g, $_g1, $c, $cexpected, $cvalue, $ebytes, $expected, $i, $status, $texpected, $tvalue, $value, $vbytes));
								{
									$GLOBALS['%s']->pop();
									return false;
								}
			...
			if(Std::is($expected, _hx_qtype("Hash")) || Std::is($expected, _hx_qtype("IntHash"))) {
				if($status->recursive || $status->path === "") {
					...
					if($keys->length !== $vkeys->length) {
						$status->error = "expected " . $keys->length . " keys but they were " . $vkeys->length . (utest_Assert_12($c, $cexpected, $cvalue, $expected, $keys, $status, $texpected, $tvalue, $value, $vkeys));
						{
							$GLOBALS['%s']->pop();
							return false;
						}
			...
			if(utest_Assert::isIterator($expected, false)) {
				if($status->recursive || $status->path === "") {
					...
					if($evalues->length !== $vvalues->length) {
						$status->error = "expected " . $evalues->length . " values in Iterator but they were " . $vvalues->length . (utest_Assert_17($c, $cexpected, $cvalue, $evalues, $expected, $status, $texpected, $tvalue, $value, $vvalues));
						{
							$GLOBALS['%s']->pop();
							return false;
						}
			...
			if(utest_Assert::isIterable($expected, false)) {
				if($status->recursive || $status->path === "") {
					...
					if($evalues->length !== $vvalues->length) {
						$status->error = "expected " . $evalues->length . " values in Iterable but they were " . $vvalues->length . (utest_Assert_20($c, $cexpected, $cvalue, $evalues, $expected, $status, $texpected, $tvalue, $value, $vvalues));
						{
							$GLOBALS['%s']->pop();
							return false;
						}
		...
		case 4:
		{
			...
			if(utest_Assert::isIterator($expected, true)) {
				...
				if($status->recursive || $status->path === "") {
					...
					if($evalues->length !== $vvalues->length) {
						$status->error = "expected " . $evalues->length . " values in Iterator but they were " . $vvalues->length . (utest_Assert_31($evalues, $expected, $status, $texpected, $tvalue, $value, $vvalues));
						{
							$GLOBALS['%s']->pop();
							return false;
						}
			...
			if(utest_Assert::isIterable($expected, true)) {
				...
				if($status->recursive || $status->path === "") {
					...
					$vvalues = Lambda::harray($value);
					if($evalues->length !== $vvalues->length) {
						$status->error = "expected " . $evalues->length . " values in Iterable but they were " . $vvalues->length . (utest_Assert_35($evalues, $expected, $status, $texpected, $tvalue, $value, $vvalues));
						{
							$GLOBALS['%s']->pop();
							return false;
						}
		...
		case 8:
		{
			$裨mp = utest_Assert_37($expected, $status, $texpected, $tvalue, $value);
			$GLOBALS['%s']->pop();
			return $裨mp;
		}break;
		}
		{
			$裨mp = utest_Assert_38($expected, $status, $texpected, $tvalue, $value);
			$GLOBALS['%s']->pop();
			return $裨mp;
		}
		$GLOBALS['%s']->pop();
...
function utest_Assert_10(&$c, &$cexpected, &$cvalue, &$expected, &$status, &$texpected, &$tvalue, &$value) {
	$製pos = $GLOBALS['%s']->length;
	{
		$GLOBALS['%s']->push("utest.Assert::sameAs@289");
...
function utest_Assert_11(&$c, &$cexpected, &$cvalue, &$expected, &$keys, &$status, &$texpected, &$tvalue, &$value) {
	$製pos = $GLOBALS['%s']->length;
	{
		$GLOBALS['%s']->push("utest.Assert::sameAs@290");
		$製pos2 = $GLOBALS['%s']->length;
		{
			$裨mp = $value->keys();
			$GLOBALS['%s']->pop();
...
function utest_Assert_15(&$c, &$cexpected, &$cvalue, &$expected, &$status, &$texpected, &$tvalue, &$value) {
	$製pos = $GLOBALS['%s']->length;
	{
		$GLOBALS['%s']->push("utest.Assert::sameAs@311");
		$製pos2 = $GLOBALS['%s']->length;
		{
			$GLOBALS['%s']->pop();
			return $expected;
...
function utest_Assert_16(&$c, &$cexpected, &$cvalue, &$evalues, &$expected, &$status, &$texpected, &$tvalue, &$value) {
	$製pos = $GLOBALS['%s']->length;
	{
		$GLOBALS['%s']->push("utest.Assert::sameAs@312");
		$製pos2 = $GLOBALS['%s']->length;
		{
			$GLOBALS['%s']->pop();
			return $value;
...
function utest_Assert_29(&$expected, &$status, &$texpected, &$tvalue, &$value) {
	$製pos = $GLOBALS['%s']->length;
	{
		$GLOBALS['%s']->push("utest.Assert::sameAs@427");
		$製pos2 = $GLOBALS['%s']->length;
		{
			$GLOBALS['%s']->pop();
			return $expected;
...
function utest_Assert_30(&$evalues, &$expected, &$status, &$texpected, &$tvalue, &$value) {
	$製pos = $GLOBALS['%s']->length;
	{
		$GLOBALS['%s']->push("utest.Assert::sameAs@428");
		$製pos2 = $GLOBALS['%s']->length;
		{
			$GLOBALS['%s']->pop();
			return $value;
...
function utest_Assert_44($f, $timeout) {
	$製pos = $GLOBALS['%s']->length;
	{
		$GLOBALS['%s']->push("utest.Assert::typeToString@665");
		$製pos = $GLOBALS['%s']->length;
		{
...
function utest_Assert_45($f, $timeout) {
	$製pos = $GLOBALS['%s']->length;
	{
		$GLOBALS['%s']->push("utest.Assert::typeToString@676");
		$製pos = $GLOBALS['%s']->length;
		{
			$裨mp = array(new _hx_lambda(array(&$f, &$timeout), "utest_Assert_47"), 'execute');
			$GLOBALS['%s']->pop();
			return $裨mp;
		}
		$GLOBALS['%s']->pop();
...
function utest_Assert_46(&$f, &$timeout) {
	$製pos = $GLOBALS['%s']->length;
	{
		$GLOBALS['%s']->push("utest.Assert::typeToString@666");
		$製pos2 = $GLOBALS['%s']->length;
		$GLOBALS['%s']->pop();
	}
}
function utest_Assert_47(&$f, &$timeout, $e) {
	$製pos = $GLOBALS['%s']->length;
	{
		$GLOBALS['%s']->push("utest.Assert::typeToString@677");
		$製pos2 = $GLOBALS['%s']->length;
		$GLOBALS['%s']->pop();
	}
}
