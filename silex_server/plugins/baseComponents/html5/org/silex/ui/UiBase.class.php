...
class org_silex_ui_UiBase extends org_silex_ui_MovieClip {
	...
	public function getHTML() {
		...
		if($this->iconIsIcon && ($this->iconDeeplinkName === "" || $this->iconDeeplinkName === null)) {
			...
			if($this->iconIsIcon) {
				$res .= "<a href='?/" . $this->__silex__publicationName__ . "/" . $this->__silex__layerPath__->join("/") . "/" . $this->iconDeeplinkName . "&amp;format=html&amp;selectedIcon=" . rawurlencode($this->__silex__layerPath__->join(".") . "." . $this->playerName) . "'>";
			} else {
