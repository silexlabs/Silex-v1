/*This file is part of Silex - see http://projects.silexlabs.org/?/silex

Silex is © 2010-2011 Silex Labs and is released under the GPL License:

This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License (GPL) as published by the Free Software Foundation; either version 2 of the License, or (at your option) any later version. 

This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.

To read the license please visit http://www.gnu.org/copyleft/gpl.html
*/
package org.silex.serverApi;

import org.silex.serverApi.externs.LoggerExtern;

class Logger
{
	private var loggerExtern : LoggerExtern;
	
	public function new()
	{
		loggerExtern = new LoggerExtern();
	}
	
	/**
	*  Returns the Log Level as a String.
	*/
	public function getLogLevel(loggerName : String) : String
	{
		return loggerExtern.getLogLevel(loggerName);
	}
	/**
	*  Prints debug message.
	*/
	public function debug(message : Dynamic) : Void
	{
		return loggerExtern.debug(message);
	}
	/**
	*  Prints info message.
	*/
	public function info(message : Dynamic) : Void
	{
		return loggerExtern.info(message);
	}
	/**
	*  Prints error message.
	*/
	public function err(message : Dynamic) : Void
	{
		return loggerExtern.err(message);
	}
	/**
	*  Prints critical error message.
	*/
	public function crit(message : Dynamic) : Void
	{
		return loggerExtern.crit(message);
	}
	/**
	*  Prints alert message.
	*/
	public function alert(message : Dynamic) : Void
	{
		return loggerExtern.alert(message);
	}
	/**
	*  Prints emergency message.
	*/
	public function emerg(message : Dynamic) : Void
	{
		return loggerExtern.emerg(message);
	}
}